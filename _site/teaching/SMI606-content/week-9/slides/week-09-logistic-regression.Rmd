...
.tuos_purple[Week 9: Logistic Regression — Part I]
#.tuos_purple[What is logistic regression and why learn it?]
...
### .tuos_purple[Multiple linear regression is great... But what if the thing we're interested in isn't on a continuous scale?]
...
Every __1 year increase in age__ is associated with a __1.87 percentage point increase__ in the probability of owning a TV license.
...
.tuos_purple[Week 9: Logistic Regression — Part II]
#.tuos_purple[How does logistic regression work?]
...
.tuos_purple[Week 9: Logistic Regression — Part III]
#.tuos_purple[Estimating, interpreting, and communicating logistic regression in `R`.]
...
This __odds ratio__ tells us the multiplicative change in the odds for a 1-unit increase — in other words __how many more times likely is it that Y = 1 when when the independent variable increases by 1__.
...
This means that __the odds of believing in alien conspiracy theories decreases by 84.2% among those with higher education degrees, compared to those with no qualifications__ (1 - exp(-1.844309) or 1 - 0.158)
...
```{r, fig.height=3.8, fig.width=5, out.height=380, out.width=500}
...
.tuos_purple[Week 9: Logistic Regression — Part IV]
#.tuos_purple[Logistic regression model fit and accuracy.]
...
## .tuos_purple[But wait, what happened to our R-squared statistic? How good is our model overall?]
...
confusionMatrix(
  data = fct_relevel(factor( conspiracy_data$pred_consp_alien ), 
                     ref = "0"),
  reference = fct_relevel(factor( conspiracy_data$consp_alien ), 
                          ref = "0"),
)
...
.tuos_purple[Week 9: Logistic Regression — Part V]
#.tuos_purple[Logistic regression assumptions.]
...
```{r, out.width=700, out.height=350, fig.width=7, fig.height=3.5}
...
```{r, echo = FALSE, out.width=700, out.height=450, fig.width=7, fig.height=4.5}
...
.tuos_purple[Week 9: Logistic Regression — Summary & Practical]
#.tuos_purple[Summary & Practical.]
