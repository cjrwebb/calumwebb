...
style_duo_accent(
  ...
  header_h3_font_size = "1.5rem", code_font_size = "0.7rem"
)
...
idaci %>% # My data
  ...
  geom_histogram(aes(x = idaci), col = "darkgrey", fill = "transparent") + # create a histogram
...
## When a continuous/integer variable is __.tuos_purple[not]__ normally distributed (for example, if it is skewed), the mean, median, and mode will often not align and some will not end up on the 'most likely value'. 

## This is why it's __always__ important to visualise your data.
...
### .tuos_purple[However, we know from the example of income we saw before that sometimes variables are not normally distributed. How can we summarise their distribution?]
...
hse_cleaned %>%
  ggplot() +
  geom_histogram(aes(x = eqvinc), col = "darkgrey", fill = "transparent", binwidth = 5000) +
  stat_function(fun = function(x) 
      dnorm(x, mean = mean(hse_cleaned$eqvinc, na.rm=TRUE), 
                       sd = sd(hse_cleaned$eqvinc, na.rm = TRUE)) * bw * n_obs) +
  geom_vline(xintercept = mean(hse_cleaned$eqvinc, na.rm = TRUE), col = "darkgrey", size = 0.75) +
  #scale_y_continuous(limits = c(0, 31)) +
...
hse_cleaned2 %>%
  ggplot() +
  geom_histogram(aes(x = eqvinc, fill = eqvinc_3sd), col = "darkgrey", binwidth = 5000, alpha = 0.4) +
  stat_function(fun = function(x) 
      dnorm(x, mean = mean(hse_cleaned$eqvinc, na.rm=TRUE), 
                       sd = sd(hse_cleaned$eqvinc, na.rm = TRUE)) * bw * n_obs) +
  geom_vline(xintercept = mean(hse_cleaned$eqvinc, na.rm = TRUE), col = "darkgrey", size = 0.75) +
...
hse_cleaned2 %>%
  ggplot() +
  geom_histogram(aes(x = eqvinc, fill = eqvinc_95q), col = "darkgrey", binwidth = 5000, alpha = 0.4) +
  stat_function(fun = function(x) 
      dnorm(x, mean = mean(hse_cleaned$eqvinc, na.rm=TRUE), 
                       sd = sd(hse_cleaned$eqvinc, na.rm = TRUE)) * bw * n_obs) +
  geom_vline(xintercept = mean(hse_cleaned$eqvinc, na.rm = TRUE), col = "darkgrey", size = 0.75) +
